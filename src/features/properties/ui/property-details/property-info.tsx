...
export function PropertyInfo({ property }: PropertyInfoProps) {
  ...
  const p_val = property.price ?? property.price_val;

  const price = p_min
    ? `${Number(p_min).toLocaleString(locale)} - ${p_max ? Number(p_max).toLocaleString(locale) : ""}`
    : p_val
      ? Number(p_val).toLocaleString(locale)
      : null;

  return (
    <div className="space-y-8" dir={isRtl ? "rtl" : "ltr"}>
      {/* Header & Price Section */}
      <div className={cn("flex flex-col md:flex-row md:items-start md:justify-between gap-6", isRtl ? "text-right" : "text-left")}>
        <div className="flex-1">
          <div className={cn("mb-3 flex flex-wrap gap-2", isRtl ? "justify-start" : "justify-start")}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 px-6 py-4 rounded-2xl bg-white border border-gray-100 shadow-sm">
        {(property.rooms || property.bedrooms) && (
          <div className={cn("flex items-center gap-3 border-gray-100 pr-4 md:justify-center", isRtl ? "border-l last:border-0 pl-4 pr-0" : "border-r last:border-0")}>
            ...
            <div className={cn(isRtl ? "text-right" : "text-left")}>
              ...
              <p className="font-bold text-gray-900">
                {Number(property.rooms || property.bedrooms).toLocaleString(locale)}
              </p>
            </div>
          </div>
        )}
        {property.bathrooms && (
          <div className={cn("flex items-center gap-3 border-gray-100 pr-4 md:justify-center", isRtl ? "border-l last:border-0 pl-4 pr-0" : "border-r last:border-0")}>
            ...
            <div className={cn(isRtl ? "text-right" : "text-left")}>
              ...
              <p className="font-bold text-gray-900">{Number(property.bathrooms).toLocaleString(locale)}</p>
            </div>
          </div>
        )}
        {property.area && (
          <div className={cn("flex items-center gap-3 border-gray-100 pr-4 md:justify-center", isRtl ? "border-l last:border-0 pl-4 pr-0" : "border-r last:border-0")}>
            ...
            <div className={cn(isRtl ? "text-right" : "text-left")}>
              ...
              <p className="font-bold text-gray-900">
                {Number(property.area).toLocaleString(locale)}{" "}
        ...
        {(property.property_age !== undefined ||
          ...
          <div className="flex items-center gap-3 md:justify-center">
            ...
            <div className={cn(isRtl ? "text-right" : "text-left")}>
              ...
              <p className="font-bold text-gray-900">
                {Number(property.property_age ?? property.propertyAge).toLocaleString(locale)}{" "}
          ...
          {property.description && (
            <div className="space-y-4">
              ...
              <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                <p className={cn("whitespace-pre-line text-gray-600 leading-relaxed text-sm md:text-base", isRtl ? "text-right" : "text-left")}>
                  {property.description}
                </p>
              </div>
            </div>
              ...
              <dl className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                {(property.usable_area || property.usableArea) && (
                  <div className={cn("flex flex-col gap-1", isRtl ? "text-right" : "text-left")}>
                    ...
                    <dd className="font-medium text-gray-900 border-b border-gray-50 pb-2">
                      {Number(property.usable_area || property.usableArea).toLocaleString(locale)} {t("sqm")}
                    </dd>
                  </div>
                )}
                {property.balconies !== undefined && (
                  <div className={cn("flex flex-col gap-1", isRtl ? "text-right" : "text-left")}>
                    ...
                    <dd className="font-medium text-gray-900 border-b border-gray-50 pb-2">
                      {Number(property.balconies).toLocaleString(locale)}
                    </dd>
                  </div>
                )}
                {property.garages !== undefined && (
                  <div className={cn("flex flex-col gap-1", isRtl ? "text-right" : "text-left")}>
                    ...
                    <dd className="font-medium text-gray-900 border-b border-gray-50 pb-2">
                      {Number(property.garages).toLocaleString(locale)}
                    </dd>
                  </div>
                )}
              </dl>
